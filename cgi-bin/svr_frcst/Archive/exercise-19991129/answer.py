...
import cgi, pg, functs, style, string, time
...
def update_time(key, secs, bonus_pts):
	...
	if int(last_time) < int(secs):
		...
		update = admindb.query("UPDATE users set last_time = '"+str(secs)+"', bonus_points = '"+str(bonus_pts)+"' WHERE userid = '"+str(key)+"' ")
...
def Main():
	...
	print '<P><a href="/cgi-bin/svr_frcst/exercise/'+refer+'?secs='+str(secs)+'&key='+key+'&case_num='+case_num+'"><H3 align="right">Click Here to proceed</H3></a>'
