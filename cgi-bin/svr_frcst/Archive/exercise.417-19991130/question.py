...
def Main(interval, case_num, secs, key, refer):
	secs_tuple = time.localtime(secs)
	this_interval = case_num[0]+str(interval)
	special = mydb.query("SELECT * from spec_417 WHERE ticks = '"+str(secs)+"' ").getresult()
	standard = mydb.query("SELECT * from gen_417 WHERE interval_num = '"+this_interval+"' ").getresult()
	...
	print '<form method="POST" action="/cgi-bin/svr_frcst/exercise.417/answer.py">'
