...
def Main():
	...
	answers = ldb.query("SELECT * from "+usersTable+" WHERE userid = '"+str(key)+"' ").getresult()
	
	try:
		state = answers[0][5]	
		etime = answers[0][4]
	except:
		style.SendError("This page is un-reloadable, sorry.")
	if case_num[0] == "s":
		torn_txt, hail_txt, rain_txt = "No", "No", "No"
	...
	now_tuple = time.localtime(secs - 20*3600)
	...
	print '<TABLE align="CENTER" bgcolor="black" cellpadding="2" border="0" width="100%">'
        ...
        fileRef = "/home/httpd/html/archivewx/cases/results/"+case_num+".gif"                
        if os.path.isfile(fileRef):
                print '<img src="http://www.pals.iastate.edu/archivewx/cases/results/'+case_num+'.gif">'                
        else:
...
#		print 'Visual image for today is not available'
	...
	print '<BR><a href="/cgi-bin/svr_frcst/classes/index.py?className='+className+'">Class Forecast Homepage</a>'
	...
	mailresults.Main(ldb, secs, key, case_num, str_date, str( int(total) - int(bonus_pts) ), str(bonus_pts) , className)
