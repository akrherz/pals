...
import cgi, style, time, functs, string
from pgext import *

mydb = connect('arch_hourly')
...
#			print '</TR></TABLE><TABLE><TR>'
		...
		if i == (intervals -1):
			print '<TD valign="top">'
			print '<form method="POST" action="/cgi-bin/svr_frcst/general/hourly_pm.py">'
			...
			print '<TD valign="top">'
			print '<form method="POST" action="/cgi-bin/svr_frcst/general/hourly_pm.py" name="'+str(i)+'">'
...
#		mk_before_links(now + 60)
		print '<BR><CENTER>'
		print '<form method="POST" action="/cgi-bin/svr_frcst/general/results.py">'
