...
import time, style, cgi, pg, functs, string, svr_frcst
...
def mk_top(local, key):
	query = admindb.query("SELECT name from sessions WHERE key = '"+key+"' ").getresult()
...
def update_db(key, name, email):
	select = admindb.query("SELECT email, key from sessions WHERE email = '"+email+"' ").getresult()
	...
	try:
		...
		update = admindb.query("UPDATE sessions SET last_time = '0', name = '"+name+"', email = '"+email+"' WHERE key ~ '"+key+"' ")
		return key	
...
def Main():
	...
	section("Introduction:", mk_intro(secs, local) )
