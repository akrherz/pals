...
def Main(ldb, secs, key, case_num, str_date, forecast_pts, bonus_pts, className):
	...
	for i in range(len(sections)):
		...
		this_answer = admindb.query("SELECT ans from gen_answers_417 WHERE case_num = '"+case_num+"' and q_id = '"+ticks+"' ").getresult()
	...
	instructorEmail = admindb.query("SELECT instructor_email from classes WHERE class_abv = '"+className+"' ").getresult()[0][0]
