...
def mk_before(ldb, case_num, key, this_interval, className):
	tableName = "users"
	last_time = int( ldb.query("SELECT last_time from "+tableName+" WHERE userid = '"+str(key)+"' "). getresult()[0][0] )
...
def been_here(ldb, key, secs, className):
	tableName = "users"
        update = ldb.query("SELECT last_time from "+tableName+" WHERE userid = '"+str(key)+"' "). getresult()
...
def Main():
	...
	if not been_here(ldb, key, secs, className):
		question.Main(interval, case_num, secs, key, "hourly_pre.py", className)	# Looks for generic and specific questions to give the user
	...
	dir_format = time.strftime("/archivewx/data/%Y_%m_%d/", time.localtime(secs))
        ...
        functs.db_comments_417(secs, secs_tuple, "comments", "News and Notes:", className)
        print '<BR></TD></TR>\n\n'
