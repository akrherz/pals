...
def update_time(key, secs, bonus_pts):
	queryTest = mydb.query("SELECT * from users WHERE userid = '"+str(key)+"' and grade_secs = "+secs+" ").getresult()
#	last_time = mydb.query("SELECT last_time from users WHERE userid = '"+str(key)+"' ").getresult()[0][0]
#	if int(last_time) == int(secs) :
	if len(queryTest) == 0:
		bonus_pre = mydb.query("SELECT bonus_points from users WHERE userid = '"+str(key)+"' ").getresult()[0][0]
		bonus_pts = int(float(bonus_pre)) + bonus_pts

		update = mydb.query("UPDATE users set last_time = '"+str(secs)+"', grade_secs = '"+str(secs)+"', bonus_points = '"+str(bonus_pts)+"' WHERE userid = '"+str(key)+"' ")
