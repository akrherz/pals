...
# 11-12-99: Now it is time to get down to business::
...
#		print "<BR>",this_time, start_secs, noon_secs, this_time- start_secs, this_time - noon_secs, now_tuple, "<BR>"
		if this_time >= start_secs and this_time != noon_secs and this_time <= last_time:
			...
			print '<a href="'+scriptBase+'/hourly_pre.py?interval='+this_interval+'&case_num='+case_num+'&secs='+str(this_time)+'&key='+str(key)+'">'
			print '<img src="/gen/hour.php3?label='+icon_ref+'&font_size=25" border="0"></a>'
		elif this_time == noon_secs:
		        print '<a href="'+scriptBase+'/noon.py?interval='+str(int(this_interval)+1)+'&key='+str(key)+'&case_num='+case_num+'&secs='+str(next_secs)+'">'
			print '<img src="/gen/hour.php3?label=Make%20Forecast&font_size=25" border="0"></a>'
			noMore = 1
			break
	if not noMore:
	        print '<a href="'+scriptBase+'/hourly_pre.py?interval='+str(int(this_interval)+1)+'&key='+str(key)+'&case_num='+case_num+'&secs='+str(next_secs)+'">'
...
def Main():
	...
	if not been_here(key, secs):
		question.Main(interval, case_num, secs, key, "hourly_pre.py")	# Looks for generic and specific questions to give the user
	...
	dir_format = time.strftime("/archivewx/data/%Y_%m_%d/", time.localtime(secs))
        ...
        svr_frcst.db_comments_417(secs, secs_tuple, "comments", "News and Notes:")
        print '<BR></TD></TR>\n\n'
