...
def Main(secs, key, case_num, str_date, forecast_pts, bonus_pts):
	...
	for i in range(len(sections)):
		...
		if len(this_answer) == 0:
			this_answer = admindb.query("SELECT answer from spec_417 WHERE ticks = '"+ticks+"' ").getresult()
		...
		try:
			...
			if len(this_answer_txt) == 0:
				this_answer_txt = admindb.query("SELECT "+option+" from spec_417 WHERE ticks = '"+ticks+"' ").getresult()
