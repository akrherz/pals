...
def enter_text(key, question, ans, cor_ans, q_id, bonus_pts):
	...
	for quest in qs:
		...
		if form.has_key(quest):
			...
			if form.has_key(quest+"text"):			# We have a text question
				...
				bogus = enter_text(key, this_question, text_ans, "T", q_id, 0)
				...
				if len(str(q_id)) > 5:
					this_answer = admindb.query("SELECT answer from spec_417 WHERE ticks = '"+q_id+"' ").getresult()
					option_wanted = "option"+this_answer[0][0]
					ans_txt = admindb.query("SELECT "+option_wanted+" from spec_417 WHERE ticks = '"+q_id+"' ").getresult()
					comments = admindb.query("SELECT cor_comments, wro_comments from spec_417 WHERE ticks = '"+q_id+"' ").getresult()
				else: 
					...
					comments = my417db.query("SELECT cor_comments, wro_comments from gen_answers_417 WHERE case_num = '"+case_num+"' and q_id = '"+q_id+"' ").getresult()
				...
				if this_answer == "NA":
					...
					if string.lower(this_answer) == string.lower(this_option): 	# Take care of sloppy db entry routines :)
						...
						print "<P><B><i>Dr Gallus's response =></i></b>" +comments[0][0]
				try:
					bonus_pts = enter_text(key, this_question, this_option_txt, this_answer[0][0], q_id, bonus_pts)
				except:	
					bonus_pts = enter_text(key, this_question, this_option_txt, "N", q_id, bonus_pts)
