...
import cgi, style, time, string, pg
...
#	table_name = string.lower(time.strftime("%b%d%Y", now_tuple))
	results = answersdb.query("Select * from "+caseNum+"")
...
def grade_state(state, caseNum):
	states = answersdb.query("SELECT * from "+caseNum+" WHERE state = '"+state+"' ").getresult()
	if len(states) > 0:
		return 10
	return 0

def grade_time(state, etime, caseNum):
	states = answersdb.query("SELECT * from "+caseNum+" WHERE state = '"+state+"' and etime = '"+etime+"' ").getresult()
	if len(states) > 0:
		return 10
	return 0
...
def grade_summer(state, torn_guess, hail_guess, rain_guess, caseNum, etime):
	res_list = answersdb.query("SELECT type from "+caseNum+" WHERE state = '"+state+"' and etime = '"+etime+"' ").getresult() 
...
def grade_winter(state, six_guess, twelve_guess, freeze_guess, caseNum):
	res_list = answersdb.query("SELECT type from "+caseNum+" WHERE state = '"+state+"' ").getresult() 
...
def Main():
	...
	answers = admindb.query("SELECT * from users WHERE userid = '"+str(key)+"' ").getresult()
