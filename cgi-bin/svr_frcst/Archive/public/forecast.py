...
import cgi, style, time, functs, svr_frcst
...
def Main():
	...
	local_tuple = (now_tuple[0], now_tuple[1], now_tuple[2], 12, now_tuple[4], now_tuple[5], now_tuple[6], now_tuple[7], now_tuple[8] )
	local_secs = time.mktime(local_tuple)

	print '<form method="POST" action="hourly_pm.py">'
	print '<input type="hidden" name="gmt_secs" value="'+str(now)+'">'
