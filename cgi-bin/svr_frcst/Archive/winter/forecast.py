...
def chk_allready(key, secs):
	attempt = admindb.query("SELECT state from sessions_417 WHERE key = '"+str(key)+"' ").getresult()

	if len(attempt[0][0]) > 0:
		print 'Content-type: text/html \n\n'
		print '<HTML><HEAD>'
	        print '<meta http-equiv="Refresh" content="1; URL=hourly_pm.py?secs='+str(secs)+'&key='+str(key)+'">'
		print '</HEAD>'
