...
# This is the hourly script for the excercise
...
def mk_before_links(now, key, casenum):
	# It is important to note that the time gets incremented automatically when first imported into code

	now_tuple = time.localtime(now)
	six_tuple = (now_tuple[0], now_tuple[1], now_tuple[2], 0, now_tuple[4], now_tuple[5], now_tuple[6], now_tuple[7], now_tuple[8])
	six = time.mktime(six_tuple)
	intervals = int( (now - six) / 10800 )
	...
	for i in range(intervals):
		...
		if i == (intervals - 1):
			...
			print '<a href="hourly.py?casenum='+casenum+'&secs='+str(secs)+'&key='+str(key)+'"><img src="/icons/'+str(i+7)+'am.gif" align="center" BORDER="0"></a> '


def mk_after_links(now, key, casenum):
	# It is important to note that the time gets incremented automatically when first imported into code

	now_tuple = time.localtime(now)
	twelve_tuple = (now_tuple[0], now_tuple[1], now_tuple[2], 23, now_tuple[4], now_tuple[5], now_tuple[6], now_tuple[7], now_tuple[8])
	twelve = time.mktime(twelve_tuple)
	intervals = int( (twelve - now) / 10800 )

	for i in range(intervals):
		secs = now + i*10800
		if i == 0:
			print '<a href="hourly.py?casenum='+casenum+'&secs='+str(now)+'&key='+str(key)+'"><img src="/icons/next_hour.gif" BORDER="0" align="CENTER"></a>'
		else:
...
def Main():
	...
	now = secs + 10800 		# So that it is now the next hour
	now_tuple = time.localtime(now)	
...
#	functs.check_key(key, now, "hourly_am.py")
	...
	mk_before_links(now, key, casenum)
	if now_tuple[3] == 12:
		print '<a href="forecast.py?casenum='+casenum+'&secs='+str(now)+'&key='+str(key)+'"><img src="/icons/make_forecast.gif" align="CENTER"></a>'
	else:
		mk_after_links(now, key, casenum)
	print '</TD></TR>'
