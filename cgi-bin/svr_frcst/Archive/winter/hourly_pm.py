...
import cgi, style, time, functs, string, question, pg
...
#			print '</TR></TABLE><TABLE><TR>'
		...
		if i == (intervals -1):
			print '<TD valign="top">'
			print '<form method="POST" action="/cgi-bin/svr_frcst/mt417/hourly_pm.py">'
			...
			print '<TD valign="top">'
			print '<form method="POST" action="/cgi-bin/svr_frcst/mt417/hourly_pm.py" name="'+str(i)+'">'
...
#	functs.check_key(key, now, "hourly_pm.py")
	...
	annote(safe_now, now_tuple, "events")
	print '<BR>&nbsp;</TD></TR>'
	...
	print '<TR bgcolor="white"><TD><BR></TD><TD colspan="2">'
	annote(safe_now, now_tuple, "analysis")
	print '<BR>&nbsp;</TD></TR>'
...
#		mk_before_links(now + 60)
		print '<BR><CENTER>'
		print '<form method="POST" action="/cgi-bin/svr_frcst/mt417/results.py">'
