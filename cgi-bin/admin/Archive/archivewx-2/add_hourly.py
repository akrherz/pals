...
def add_entry(day, hour, comments, analysis):
	comments = regsub.gsub("'","&180;", comments)
	analysis = regsub.gsub("'","&180;", analysis)
	delete = mydb.query("delete from "+day+" where ztime = '"+hour[0:2]+"'")
	insert = mydb.query("insert into "+day+" values('" + hour+"','"+comments+"','"+analysis+"')")
	print 'DONE'
