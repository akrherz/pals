...
import time, os, sys, time, regsub, string, style, pg, DateTime

def dbComments(nowDate, colName, secHead, zswitch):
	print '<TABLE align="CENTER" bgcolor="black" cellpadding="2" border="0" width="100%">'
        ...
        print '<TR><TD><font color="blue" size="4" face="ARIAL"><B>'+secHead+'</B></font></TD></TR>'
	...
	mydb = pg.connect('severe2', 'localhost', 5432)
        ...
        try:
                select = mydb.query("SELECT "+colName+" from annotations where validtime = '"+nowDate+"' and casenum ~* '"+zswitch+"' ").getresult() 
        except ValueError:
