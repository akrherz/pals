...
import cgi, style, time, string, pg, os, sys, functs, mailresults
...
def Main():
	...
	answers = advdb.query("SELECT * from users WHERE userKey = '"+str(userKey)+"' ").dictresult()
	
	state = answers[0]["state"]	
	etime = answers[0]["etime"]

	# We have a summer case
	if caseNum[0] == "s":
		torn_txt, hail_txt, rain_txt = "No", "No", "No"
	...
	print """<P>Congratulations on finishing this exercise.  Listed below is a textual and graphical representation of what 
	has happned during this case.<BR><BR>"""
	...
	mailresults.Main(userKey, caseNum, str( int(total) - int(bonus_pts) ), str(bonus_pts) , className)
