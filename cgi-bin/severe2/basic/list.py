...
import pg, cgi, sys, time, functs, SEVERE2, mx.DateTime
...
#	mydb.query("SET DateStyle to 'Postgres'")
	cases = mydb.query("SELECT casenum from basiccases WHERE caseNum ~* '"+caseType+"'  ").getresult()
	cases.sort()
#	print '<MULTICOL COLS="3">'
	for i in range(len(cases)):
		thisCase = cases[i][0]
		startTime = mydb.query("SELECT startTime from cases WHERE caseNum = '"+thisCase+"' ").getresult()

		startDate = mx.DateTime.ISO.ParseDateTimeGMT(startTime[0][0])
		print '<a href="/cgi-bin/severe2/basic/index.py?caseNum='+thisCase+'">'+startDate.strftime("%d %B %Y")+'</a>'
		print '&nbsp; &nbsp; &nbsp; &nbsp; '
		if (i%3 == 0 and i != 0):
			print '<BR>'
