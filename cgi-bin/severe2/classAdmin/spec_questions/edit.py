...
import time, cgi, style, pg, DateTime, os

mydb = pg.connect('severe2_adv', 'localhost', 5432)
mydb.query("SET TIME ZONE 'GMT'")
table_str = "questions_custom"
className = os.environ["REMOTE_USER"]

def add_entry(ticks):
	tester = mydb.query("SELECT validTime from "+table_str+" where validTime = '"+ticks+"' and className = '"+className+"' ").getresult()
	if len(tester) == 0:
		create = mydb.query("INSERT into "+table_str+"(validTime, className) values('"+ticks+"','"+className+"')")
...
def Main():
	...
	entry = mydb.query("SELECT * from "+table_str+" WHERE validTime = '"+strTicks+"' and className = '"+className+"' ").dictresult()
	...
	optiong = entry[0]["optiong"]
	optionh = entry[0]["optionh"]
